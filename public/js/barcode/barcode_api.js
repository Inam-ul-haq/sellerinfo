        ...
        digitToCssRenderer: function ($container, settings, digit, hri, mw, mh) {
            ...
            for (var y = 0; y < lines; y++) {
                ...
                for (var x = 0; x < columns; x++) {
                    ...
                    if (newval == 2) { newval = newval + 4; } else { newval = newval + 1; }
                    if (current == digit[y][x]) {
                        len++;
                    } else {
                        content += (current == '0' ? bar0 : bar1).replace("&W", newval);
                        current = digit[y][x];
                        len = 1;
                    }
                }
            ...
            if (settings.showHRI) {
                content += "<div style=\"clear:both; text-align:center; width: 40%; background-color: " + settings.bgColor + "; color: " + settings.color + "; font-size: " + settings.fontSize + "px; margin-top: " + settings.marginHRI + "px;\">" + hri + "</div>";
            }
