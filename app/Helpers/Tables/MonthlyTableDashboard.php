...
class MonthlyTableDashboard
{

    public static function initMonthlyDashboardDataTable($request, $limit, $skip)
    {
        ...
        $orders = $orders->skip($skip)
            ...
            ->map(function ($order) {
                ...
                $price = Order::where('pname', $order->pname)->whereDate('created_at', '>', now()->subMonth())->get()
                    ->groupBy('pname')
                    ->mapWithKeys(function ($item, $pname) {
                        return ['price' => $item->sum('samp')];
                    })['price'];

                $quantity = Order::where('pname', $order->pname)->whereDate('created_at', '>', now()->subMonth())->get()
                    ->groupBy('pname')
                    ->mapWithKeys(function ($item, $pname) {
                        return ['quantity' => $item->sum('quantity')];
                    })['quantity'];

                $samq = Order::where('pname', $order->pname)->whereDate('created_at', '>', now()->subMonth())->get()
                    ->groupBy('pname')
                    ->mapWithKeys(function ($item, $pname) {
